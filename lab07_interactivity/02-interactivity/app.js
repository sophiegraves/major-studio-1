...
//different categories of things to filter by
...
async function dataLoad() {
  ...
  //Idk what this is but I'm sure it has to do with stuff
  ...
  rightMenu
    ...
    .html(
      state.sizeBy.menu
        .map(
          d =>
            `<input type="radio" name="sizeby" value="${d}" ${state.sizeBy.selected === d ? "checked" : ""
            }>${d}<br>`
        )
        .join("")
    )
...
function draw() {
  ...
  const checkRow = d3
    ...
    .html(
      d => `
      <input name="${d}" type="checkbox" ${state.filters.checked.indexOf(d) > -1 ? "checked" : ""
        }></input>
      <label for="${d}">${d}</label>
    `
    );
